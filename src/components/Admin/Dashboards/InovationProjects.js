...
import {
  resetInovationProject,
  updateInovationProject,
  updateOpenPopup,
  updatePopupData,
...
export default function InovationProjects() {
  ...
  const handleFormDescription = (event) => {
    dispatch(
      updateInovationProject({ ...inovationProject, description: event })
    );
  };

  const handleFormImage = async (e) => {
    dispatch(
      updateInovationProject({ ...inovationProject, image: e.target.files[0] })
    );
  };
  ...
  const validateForm =
    inovationProject.image &&
    inovationProject.title &&
    inovationProject.status &&
    inovationProject.description;
  const cancelForm =
    inovationProject.image ||
    inovationProject.title ||
    inovationProject.status ||
  ...
  const handleUpdateValidation = () => {
    const tempInovationProject = inovationProjectList.filter(
      (item) => item.id === inovationProject?.id
    );
    return (
      tempInovationProject[0]?.title !== inovationProject?.title ||
      tempInovationProject[0]?.image !== inovationProject?.image ||
      tempInovationProject[0]?.status !== inovationProject?.status ||
      tempInovationProject[0]?.description !== inovationProject?.description
    );
  };
    ...
    if (validateForm) {
      if (!edit) {
        ...
        const response = await putApi(
          "inovation_project/update/" + inovationProject.id,
          formData
        );
        if (response?.status === "Failed") {
          openPopup("error", "Network Error! Try again later.");
            ...
            <Row style={{ marginBottom: "10px" }}>
              ...
              <Col xs={24} sm={24} md={15} lg={15} xl={15}>
                <div>
                  ...
                  />
                  {edit && editImage === inovationProject.image ? (
                    <p
