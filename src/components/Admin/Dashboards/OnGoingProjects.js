...
export default function OnGoingProjects() {
  ...
  const handleFormImage = async (e) => {
    dispatch(
      updateOnGoingProject({ ...onGoingProject, image: e.target.files[0] })
    );
  };
  ...
  const handleUpdateValidation = () => {
    const tempOnGoingProject = projectList.filter(
      (item) => item.id === onGoingProject?.id
    );
    return (
      tempOnGoingProject[0]?.title !== onGoingProject?.title ||
      tempOnGoingProject[0]?.image !== onGoingProject?.image ||
      tempOnGoingProject[0]?.status !== onGoingProject?.status ||
      tempOnGoingProject[0]?.description !== onGoingProject?.description
    );
  };
    ...
    if (validateForm) {
      if (!edit) {
        ...
        const response = await putApi(
          "ongoing_project/update/" + onGoingProject.id,
          formData
        );
        if (response?.status === "Failed") {
          openPopup("error", "Network Error! Try again later.");
            ...
            <Row style={{ marginBottom: "10px" }}>
              ...
              <Col xs={24} sm={24} md={15} lg={15} xl={15}>
                <div>
                  ...
                  />
                  {edit && editImage === onGoingProject.image ? (
                    <p
