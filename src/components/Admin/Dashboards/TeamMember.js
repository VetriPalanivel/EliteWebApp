...
export default function TeamMember() {
  ...
  const handleUpdateValidation = () => {
    const tempTeamMember = teamMemberList.filter(
      (item) => item.id === teamMember?.id
    );
    return (
      tempTeamMember[0]?.name !== teamMember?.name ||
      tempTeamMember[0]?.image !== teamMember?.image ||
      tempTeamMember[0]?.role !== teamMember?.role ||
      tempTeamMember[0]?.description !== teamMember?.description
    );
  };
  ...
  const handleAddProject = async () => {
    ...
    formData.append("name", firstName + ". " + teamMember.name);
    ...
    if (validateForm) {
      if (!edit) {
        ...
        const response = await putApi(
          "team_member/update/" + teamMember.id,
          formData
        );
        if (response?.status === "Failed") {
          openPopup("error", "Network Error! Try again later.");
  ...
  return (
    <div className="researchProjects-container">
            ...
            <Row style={{ marginBottom: "10px" }}>
              ...
              <Col xs={24} sm={24} md={15} lg={15} xl={15}>
                <div>
                  ...
                  />
                  {edit && editImage === teamMember.image ? (
                    <p
      ...
      <div className="Display-FormDetails">
        <h5 className="Display-heading">ADDED EGE TEAM MEMBER DETAIL</h5>
        <Grid>
                ...
                <Card className="Form-DisplayCard-Teammember">
                  <CardContent>
                    ...
                    <div>
                      ...
                      <p className="Display-content-text-member">{item.role}</p>
                      <CardActions
