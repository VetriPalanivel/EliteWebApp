...
export default function ResearchAssistant() {
  ...
  const validateForm =
    assistantJob.title &&
    assistantJob.image &&
    assistantJob.description &&
    assistantJob.requirement &&
    assistantJob.benefit &&
    assistantJob.duration &&
    assistantJob.deadline;
  const cancelForm =
    assistantJob.title ||
    assistantJob.image ||
    assistantJob.description ||
  ...
  const handleUpdateValidation = () => {
    const tempResearchAssistant = assistantJobList.filter(
      (item) => item.id === assistantJob?.id
    );
    return (
      tempResearchAssistant[0]?.title !== assistantJob?.title ||
      tempResearchAssistant[0]?.image !== assistantJob?.image ||
      tempResearchAssistant[0]?.description !== assistantJob?.description ||
      tempResearchAssistant[0]?.requirement !== assistantJob?.requirement ||
      tempResearchAssistant[0]?.benefit !== assistantJob?.benefit ||
      tempResearchAssistant[0]?.duration !== assistantJob?.duration ||
      tempResearchAssistant[0]?.deadline !== assistantJob?.deadline
    );
  };
    ...
    if (validateForm) {
      if (!edit) {
        ...
        const response = await putApi(
          "research_assistantjob/update/" + assistantJob.id,
          formData
        );
        if (response?.status === "Failed") {
          openPopup("error", "Network Error! Try again later.");
            ...
            <Row style={{ marginBottom: "10px" }}>
              ...
              <Col xs={24} sm={24} md={15} lg={15} xl={15}>
                <div>
                  ...
                  />
                  {edit && editImage === assistantJob.image ? (
                    <p
              ...
              <Col xs={20} sm={20} md={15} lg={15} xl={15}>
                <ButtonToolbar className="confirmButton">
                  <Button
                    ...
                    id="cancel"
                    appearance="primary"
                    onClick={handleCancelAssistantJob}
                  >
                    Cancel
                  </Button>
                  {!edit ? (
