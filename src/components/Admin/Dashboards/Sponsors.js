...
export default function Sponsors() {
  ...
  const handleUpdateValidation = () => {
    const tempSponsors = sponsorsList.filter(
      (item) => item.id === sponsors?.id
    );
    return (
      tempSponsors[0]?.name !== sponsors?.name ||
      tempSponsors[0]?.image !== sponsors?.image ||
      tempSponsors[0]?.type !== sponsors?.type ||
      tempSponsors[0]?.description !== sponsors?.description ||
      tempSponsors[0]?.country !== sponsors?.country
    );
  };
