...
export default function Competetion() {
  ...
  const handleFormSelect = (event) => {
    dispatch(updateCompetetion({ ...competetion, mode: event, venue: "" }));
  };

  const handleFormDate = (event) => {
    dispatch(updateCompetetion({ ...competetion, deadline: event }));
  };
  ...
  const handleUpdateValidation = () => {
    ...
    return (
      ...
      tempCompetetion[0]?.deadline !== competetion?.deadline ||
  ...
  const handleAddProject = async () => {
    ...
    formData.append("deadline", competetion.deadline);
    ...
    if (validateForm) {
      if (!edit) {
        ...
        const response = await putApi(
          "competetion/update/" + competetion.id,
          formData
        );
        if (response?.status === "Failed") {
          openPopup("error", "Network Error! Try again later.");
  ...
  return (
    <div className="researchProjects-container">
            ...
            <Row style={{ marginBottom: "10px" }}>
              ...
              <Col xs={24} sm={24} md={15} lg={15} xl={15}>
                <div>
                  ...
                  />
                  {edit && editImage === competetion.image ? (
                    <p
      ...
      <div className="Display-FormDetails">
        <h5 className="Display-heading">ADDED COMPETETION DETAIL</h5>
